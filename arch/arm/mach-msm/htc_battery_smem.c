...
static int init_battery_settings(struct battery_info_reply *buffer) {
	...
	if(debug_mask&DEBUG_LOG)
		BATT("init_battery_settings: VREF=%d; 0.5-VREF=%d; ADC_A=%d; ADC_B=%d; htc_adc_range=%d; batt_id=%d; full_bat=%d\n", \
		batt_vref, batt_vref_half, htc_adc_a, htc_adc_b, htc_adc_range, buffer->batt_id, buffer->full_bat);
